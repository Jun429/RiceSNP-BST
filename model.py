...
import pandas as pd
import numpy as np
data_train=pd.read_csv('data2/data2/plan3/data2_plan3_50/1000_RF_data2_plan3_50_merge.csv',header=None)
X=np.array(data_train)
print(X.shape)
X_train = X.reshape(18318,250,4)
y = [1]*9159+[0]*9159
Y_train=np.array(y)
...
wd = "./RF/50_plan3_1000/AmberDeepSea/"
if os.path.isdir(wd):
    shutil.rmtree(wd)
os.makedirs(wd)
input_node = Operation('input', shape=(250,4), name="input")##1:(10728,1001,15),2:(10728,1001,20),3:(10728,1001,36),4:(10728,1001,33)
...
specs = {
    ...
    'train_env': {
        'max_episode': 10,  # has been reduced for running in colab
...
num_folds = 5
num_runs = 5
csv_file = 'RF/50_plan3_1000/val.csv'
...
for run in range(num_runs):
    ...
    for fold, (train_indexes, val_indexes) in enumerate(skf.split(X_train, Y_train)):
        ...
        result_val_fold.to_csv(f'RF/50_plan3_1000/val_results_run{run}_fold{fold}.csv', index=False)
